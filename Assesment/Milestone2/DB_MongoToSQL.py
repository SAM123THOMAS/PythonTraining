...
try:
    ...
    #Define the database
    DATABASE_URL = "sqlite:///movietest2.db"
    ...
    for movie in movie_details:
            ...
            #Create
            new_user = MovieClass(id = movie_id, movie_name = title, movie_plot=plot, movie_directors=directors, movie_languages=languages, released_date=released, movie_genres=genres)
            session.add(new_user)
    session.commit()

    #Read
    movies_read = session.query(MovieClass).all()
    df_data = pd.DataFrame(movies_read)
